Shader "hair"
{
	Properties
	{
		...
		_HairLightTex("Hair Light Tex", 2D) = "white" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (v2f i) : SV_Target
			{
				...
				//half light_atten = SHADOW_ATTENUATION(i);
				//light_atten = light_atten + (light_atten - 0.5) * _ShadowContrast;
				...
				//diff_term = diff_term + (diff_term - 0.9) * _ShadowContrast;
				...
				//half diffuse_color_term = light_atten * diff_term ;
				//half diffuse_color_uv_x = saturate(diffuse_color_term);
				//half2 diffuse_color_uv = half2(diffuse_color_uv_x, 0);
				//half3 diffuse_color = tex2D(_HairLightTex, diffuse_color_uv) * hairLineTex ;

				//half3 diffuse_color = base_diffuse_color;
